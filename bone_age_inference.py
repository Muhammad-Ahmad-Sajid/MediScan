    ...
    def forward(self, x, gender=None):
        ...
        if self.use_gender:
            if gender is None:
                ...
                gender = torch.full((features.size(0), 1), 0.5, device=features.device, dtype=features.dtype)
...
def get_tta_transforms():
    ...
    return [
        ...
        A.Compose([A.Resize(224, 224), A.Rotate(limit=(5, 5), p=1.0), A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), ToTensorV2()]),
        A.Compose([A.Resize(224, 224), A.Rotate(limit=(-5, -5), p=1.0), A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), ToTensorV2()]),
        A.Compose([A.Resize(224, 224), A.RandomBrightnessContrast(brightness_limit=(0.1, 0.1), contrast_limit=0, p=1.0), A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), ToTensorV2()]),
    ]
...
def determine_deviation(predicted_months: float, chronological_age_months: Optional[int]):
    if chronological_age_months is None:
        return None, "no_comparison", "Chronological age not provided. Unable to assess skeletal maturity deviation. Provide patient's date of birth for full assessment."
...
if __name__ == "__main__":
    ...
    try:
        ...
        if res.chronological_age_months is not None:
            print(f"Chronological Age  : {format_age_display(res.chronological_age_months)} ({res.chronological_age_months} months)")
