...
def get_clinical_recommendation(tumor_type, confidence_flag, glioma_risk_flag, glioma_prob):
    ...
    if tumor_type == "notumor":
        if confidence_flag == "clear":
            rec = (
                "No brain tumor detected on MRI.\n"
                "Brain parenchyma appears within normal limits.\n"
                "Routine follow-up as clinically indicated."
            )
        else:
            rec = (
                "No definitive tumor identified, but findings are inconclusive.\n"
        ...
        if confidence_flag == "clear":
            rec = (
                "URGENT: Brain MRI findings strongly suggest glioma.\n"
            ...
            rec = (
                "Brain MRI findings are consistent with possible glioma.\n"
        ...
        if confidence_flag == "clear":
            rec = (
                "Brain MRI findings suggest meningioma.\n"
            ...
            rec = (
                "Findings suggest possible meningioma, but confidence is limited.\n"
        ...
        if confidence_flag == "clear":
            rec = (
                "Brain MRI findings suggest pituitary tumor.\n"
            ...
            rec = (
                "Findings suggest possible pituitary abnormality.\n"
...
if __name__ == "__main__":
    ...
    rec_preview = res.clinical_recommendation.replace("\n", " ")[:80] + (
        "..." if len(res.clinical_recommendation) > 80 else ""
    )
