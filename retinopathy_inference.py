...
def get_clinical_recommendation(
    ...
    if grade == 0:
        if conf_flag == "clear":
            ...
            rec = (
                "No definitive diabetic retinopathy identified,\n"
        ...
        rec = (
            "URGENT: Proliferative diabetic retinopathy detected.\n"
...
def run_retinopathy_inference(image_path: str) -> RetinopathyResult:
    ...
    if c == 3:
        ...
        if (b == g).all() and (g == r).all():
            logger.warning(
                "Image appears grayscale. Retinopathy module expects color fundus images. Results may be unreliable."
            )
    elif c == 1:
        logger.warning(
            "Image appears grayscale. Retinopathy module expects color fundus images. Results may be unreliable."
