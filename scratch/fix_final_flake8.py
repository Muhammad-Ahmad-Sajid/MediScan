...
content = re.sub(r'except ImportError as e:\n    logger.warning\(f"([^"]+)"\)', r'except ImportError:\n    logger.warning("\1")', content)
...
content = re.sub(r'except Exception:\n        logger.error\(f"([^"]+)"\)', r'except Exception:\n        logger.error("\1")', content)
