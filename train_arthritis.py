...
def main():
    ...
    with mlflow.start_run(run_name="arthritis_v1"):
        ...
        logger.info(
            f"{'Epoch':<5} | {'Train Loss':<10} | {'Train Acc':<9} | {'Val Loss':<8} | {'Val Acc':<7} | {'Best':<4} | {'LR'}"
        )
        logger.info("-" * 75)
        ...
        for epoch in range(start_epoch, NUM_EPOCHS + 1):
            ...
            logger.info(
                f"{epoch:<5} | {train_loss:<10.4f} | {train_acc:<9.4f} | {val_loss:<8.4f} | {val_acc:<7.4f} | {best_marker:<4} | {current_lr:.2e}"
            )
            ...
            if epoch % 5 == 0:
                ...
                if recall_severe < 0.30:
                    logger.warning(
                        f"Grade 4 (Severe) recall is below 30% ({recall_severe:.2%}). Model is ignoring the rarest class."
                    )
                logger.info("-" * 75)
