...
def prepare_data(csv_path, img_dir):
    ...
    for name, split_df in [('train', train_df), ('val', val_df), ('test', test_df)]:
        logger.info(f"  {name:5s} | {len(split_df):5d} | {split_df['age_months'].min():.0f}-{split_df['age_months'].max():.0f}     | {split_df['age_months'].mean():.1f}    | {split_df['age_months'].std():.1f}")
...
def train_one_epoch(model, dataloader, optimizer, criterion):
    ...
    for batch in dataloader:
        if len(batch) == 4:
            ...
            inputs, genders, labels = inputs.to(DEVICE), genders.to(DEVICE), labels.to(DEVICE)
...
def main():
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        logger.info(f"{epoch:5d} | {train_loss:10.4f} | {train_mae:9.2f} | {val_loss:8.4f} | {val_mae:16.2f} | {val_rmse:8.2f} | {within_12:11.1f}% | {lr:.2e}")
        logger.info(f"Bone Age MAE: {val_mae:.1f} months — target below 12 months")
        
        if epoch >= 10 and val_mae > 20.0:
            logger.warning("⚠️ MAE still above 20 months. Model may not be converging properly. Check: (1) age normalization, (2) learning rate, (3) data loading.")
    ...
    labels = ['0-24 months', '25-60 months', '61-120 months', '121-180 months', '181-228 months']
    ...
    print("\nTraining Summary:")
    print(f"Best Epoch: {best_epoch} | Best MAE: {best_val_mae:.2f} | Best RMSE: {best_rmse:.2f} | Time: {total_time/60:.1f} min")
