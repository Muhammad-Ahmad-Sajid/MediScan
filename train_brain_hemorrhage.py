...
def perform_discovery():
    ...
    for root, dirs, files in os.walk(BASE_DIR):
        ...
        indent = '  ' * level
        img_count = len([f for f in files if f.lower().endswith(('.png','.jpg','.jpeg','.tif','.dcm'))])
        total += img_count
    ...
    if total < 300:
        logger.warning(f"⚠️ Extremely small dataset ({total} images). Results may not generalize. Consider combining with additional CT hemorrhage datasets before clinical deployment.")
...
def evaluate(model, dataloader, criterion):
    ...
    return epoch_loss, epoch_acc, sensitivity, specificity, all_preds, all_labels, all_probs
...
def tta_evaluate(model, dataset):
    ...
    t_orig = transforms.Compose([transforms.Resize((224, 224)), transforms.ToTensor(), transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
    t_hf = transforms.Compose([transforms.Resize((224, 224)), transforms.RandomHorizontalFlip(p=1.0), transforms.ToTensor(), transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
    t_r5 = transforms.Compose([transforms.Resize((224, 224)), transforms.RandomRotation((5, 5)), transforms.ToTensor(), transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
    t_rm5 = transforms.Compose([transforms.Resize((224, 224)), transforms.RandomRotation((-5, -5)), transforms.ToTensor(), transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
    t_zoom = transforms.Compose([transforms.Resize((240, 240)), transforms.CenterCrop(224), transforms.ToTensor(), transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
...
def run_cross_validation(X, y):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        logger.info(f"Epoch | Train Loss | Train Acc | Val Loss | Val Acc | Sensitivity | Specificity | LR")
        logger.info("-" * 85)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            logger.info(f"{epoch:5d} | {train_loss:10.4f} | {train_acc:9.4f} | {val_loss:8.4f} | {val_acc:7.4f} | {sens:11.4f} | {spec:11.4f} | {lr:.2e}")
            logger.info(f"🚨 Hemorrhage Sensitivity: {sens*100:.2f}% — target above 90%")
            
            if epoch >= 10 and sens < 0.80:
                logger.warning(f"⚠️ CRITICAL: Hemorrhage sensitivity {sens*100:.2f}% is dangerously low. This is an emergency detection module. Investigate immediately.")
    ...
    if sens < 0.90:
        ...
        logger.info("Actions: (1) Lower classification threshold to 0.35, (2) Combine with additional hemorrhage datasets, (3) Apply test-time augmentation (TTA).")
    else:
