...
def main():
    ...
    print(f"train | {train_counts[0]:8} | {train_counts[1]:6} | {train_counts[2]:10} | {train_counts[3]:9} | {len(y_train)}")
    print(f"val   | {val_counts[0]:8} | {val_counts[1]:6} | {val_counts[2]:10} | {val_counts[3]:9} | {len(y_val)}")
    print(f"test  | {test_counts[0]:8} | {test_counts[1]:6} | {test_counts[2]:10} | {test_counts[3]:9} | {len(y_test)}\n")
    ...
    with mlflow.start_run(run_name="brain_tumor_v1"):
        mlflow.log_params({
            "model": "ResNet50", "dataset": "Brain_Tumor_MRI",
            "num_classes": 4, "batch_size": BATCH_SIZE, "lr": LR
        })
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            print(f"{epoch:5} | {t_loss:10.4f} | {t_acc:9.4f} | {v_loss:8.4f} | {v_acc:7.4f} | {recalls['glioma']:13.4f} | {recalls['meningioma']:17.4f} | {recalls['pituitary']:16.4f} | {current_lr:.2e}")
            ...
            history.append({
                ...
                "train_acc": t_acc, "val_acc": v_acc, "recalls": recalls
            })
            ...
            if v_loss < best_val_loss:
                ...
                torch.save({
                    ...
                    'val_loss': v_loss, 'val_acc': v_acc, 'per_class_recall': recalls
                }, BEST_MODEL_PATH)
            else:
                epochs_no_improve += 1
    ...
    if test_recalls['glioma'] < 0.85:
        ...
        print("Consider: (1) increasing glioma class weight, (2) lowering classification threshold for glioma, (3) adding more augmentation.")
