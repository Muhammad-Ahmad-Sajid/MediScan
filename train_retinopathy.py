...
from sklearn.metrics import cohen_kappa_score, classification_report, confusion_matrix, recall_score
import numpy as np
import pandas as pd
import torch
...
def get_dataloaders(csv_file):
    ...
    for name, split_df in [('train', train_df), ('val', val_df), ('test', test_df)]:
        ...
        c0, c1, c2, c3, c4 = counts.get(0,0), counts.get(1,0), counts.get(2,0), counts.get(3,0), counts.get(4,0)
...
def train_model():
    ...
    for epoch in range(start_epoch, 31):
            if epoch == 6:
                ...
                unfreeze_all(model)
                optimizer = optim.Adam(model.parameters(), lr=scheduler.optimizer.param_groups[0]['lr'], weight_decay=1e-5)
                scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', patience=3, factor=0.5)
            ...
            logger.info(f"{epoch:5} | {train_loss:10.4f} | {train_acc*100:8.1f}% | {val_loss:8.4f} | {val_acc*100:6.1f}% | {qwk:7.4f} | {g4_recall:12.1f}% | {lr:.2e}")
            
            logger.info(f"QWK: {qwk:.4f} — target above 0.70")
            logger.info(f"Grade 4 (Proliferative) Recall: {g4_recall:.2f}% — target above 70%")
            
            if epoch >= 10:
                if qwk < 0.50: logger.warning("WARNING: QWK below 0.50 after epoch 10")
                if g4_recall < 50.0: logger.warning("WARNING: Grade 4 recall below 50% after epoch 10")
...
# Removed MLFlow to bypass Windows DLL deadlock
            ...
            if qwk > best_qwk:
                best_qwk = qwk
                patience_counter = 0
                torch.save({'state_dict': model.state_dict(), 'epoch': epoch, 'val_qwk': qwk, 'per_class_recall': recalls.tolist()}, CHECKPOINT_PATH)
            else:
                patience_counter += 1
    ...
    # Warnings
    if qwk_test < 0.70:
        logger.warning("⚠️ QWK below target. Consider: (1) ordinal regression loss, (2) label smoothing, (3) focal loss for rare classes.")
    if recalls_test[4] < 0.70:
        logger.warning("⚠️ Proliferative DR recall below threshold. These patients risk blindness without treatment. Increase Grade 4 class weight.")
